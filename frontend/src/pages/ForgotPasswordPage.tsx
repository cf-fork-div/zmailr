...
const ForgotPasswordPage: React.FC = () => {
  ...
  const handleSendCode = async () => {
    ...
    if (codeSentForCurrentCredentials) {
      const result = await authPasswordResetResend(
        sentEmail,
        turnstileRequired ? turnstileToken : undefined
      );
      setSendingCode(false);
      if (result.success) {
        setResendCooldown(60);
        if (result.deliveryHint) setDeliveryHint(result.deliveryHint);
        setSuccessHint(t('auth.forgotPasswordCodeSentHint'));
        return;
      }
      setError(result.error || t('auth.forgotPasswordSendFailed'));
      return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (code.trim().length < 6) {
      setError(t('auth.forgotPasswordEnterCode'));
      return;
    }

    if (!codeSentForCurrentCredentials) {
      setError(t('auth.forgotPasswordSendCodeFirst'));
      return;
    }

    setLoading(true);
    const result = await authPasswordResetVerify(sentEmail, code.trim());
    setLoading(false);
    if (result.success) {
      await refresh();
    ...
    <div className="login-shell relative min-h-screen flex flex-col">
      ...
      <div className="relative flex-1 flex items-center justify-center px-4 py-6">
        <div className="w-full max-w-md rounded-2xl border border-sky-200/70 dark:border-border bg-card shadow-xl shadow-sky-500/10 dark:shadow-black/20 p-6 sm:p-8">
          ...
          <p className="text-sm text-muted-foreground mt-1 mb-6">{t('auth.forgotPasswordHint')}</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">{t('auth.forgotPasswordCodeFieldHint')}</p>
            </div>
            <button
              type="submit"
              disabled={loading || sendingCode || (turnstileRequired && !turnstileToken)}
