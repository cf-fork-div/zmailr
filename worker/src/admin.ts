...
<div id="appView" class="app">
  ...
  <div id="panel-rules" class="panel">
    ...
    <div class="toolbar">
      <button class="btn btn-sm" data-tfilter="pending" onclick="setTemplateFilter('pending')">待审核</button>
      <button class="btn btn-sm" data-tfilter="approved" onclick="setTemplateFilter('approved')">已通过</button>
      <button class="btn btn-sm" data-tfilter="rejected" onclick="setTemplateFilter('rejected')">已拒绝</button>
      <button class="btn btn-sm" data-tfilter="all" onclick="setTemplateFilter('all')">全部</button>
      <button class="btn btn-sm" onclick="loadRuleTemplates()">刷新</button>
    </div>
  ...
  <div id="panel-settings" class="panel">
    <h3 class="section-title">用户 API Token</h3>
    <p class="section-desc">每位 Dashboard 用户可持有的 Bearer Token 数量上限（含已过期但未删除的 Token）</p>
    <div class="form-group"><label>Token 数量上限</label><input type="number" id="maxUserTokens" min="1" max="100" step="1" value="3"></div>
    <hr style="border:none;border-top:1px solid #27272a;margin:24px 0">
    ...
    <div class="form-group"><label>维护提示信息（可选前缀）</label><textarea id="maintMessage" placeholder="例如：系统升级中，预计 30 分钟恢复" oninput="updateMaintenancePreview()"></textarea></div>
    <p class="hint" style="margin-bottom:12px">下方勾选的阻断项会自动追加到用户端横幅，例如「暂停服务：创建新邮箱（含 API 租用）。」</p>
    <div class="form-group"><label><input type="checkbox" id="maintBlockLease" checked onchange="updateMaintenancePreview()"> 阻断 POST /api/lease</label></div>
    <div class="form-group"><label><input type="checkbox" id="maintBlockSend" checked onchange="updateMaintenancePreview()"> 阻断 POST /api/send 与 /api/user/send</label></div>
    <div class="form-group"><label><input type="checkbox" id="maintBlockMailbox" checked onchange="updateMaintenancePreview()"> 阻断创建邮箱（含 lease）</label></div>
...
function showUserModal(){document.getElementById('userModalTitle').textContent='新增用户';document.getElementById('userId').value='';document.getElementById('userUsername').value='';document.getElementById('userPassword').value='';document.getElementById('userRole').value='user';document.getElementById('userQuota').value='50';document.getElementById('userLeaseQuota').value='300';document.getElementById('userRatePlan').value='free';applyRatePlan();document.getElementById('userEnabledGroup').style.display='none';showModal('userModal')}
function editUser(id){const u=(window._users||[]).find(x=>x.id===id);if(!u)return;document.getElementById('userModalTitle').textContent='编辑用户';document.getElementById('userId').value=u.id;document.getElementById('userUsername').value=u.username;document.getElementById('userUsername').disabled=true;document.getElementById('userPassword').value='';document.getElementById('userRole').value=u.role;document.getElementById('userQuota').value=u.dailySendQuota;document.getElementById('userLeaseQuota').value=u.dailyLeaseQuota??300;document.getElementById('userRatePlan').value=detectRatePlan(u);document.getElementById('userRateLimit').value=u.rateLimitPerMin??60;document.getElementById('userRateBurst').value=u.rateLimitBurst??'';document.getElementById('userEnabled').value=u.enabled?'1':'0';document.getElementById('userEnabledGroup').style.display='block';showModal('userModal')}
async function saveUser(){const id=document.getElementById('userId').value;const burstRaw=document.getElementById('userRateBurst').value;const body={username:document.getElementById('userUsername').value,password:document.getElementById('userPassword').value,role:document.getElementById('userRole').value,dailySendQuota:parseInt(document.getElementById('userQuota').value)||50,dailyLeaseQuota:parseInt(document.getElementById('userLeaseQuota').value)||300,rateLimitPerMin:parseInt(document.getElementById('userRateLimit').value)||60,rateLimitBurst:burstRaw===''?null:parseInt(burstRaw)||null};if(id){body.enabled=document.getElementById('userEnabled').value==='1';if(!body.password)delete body.password;await api('/users/'+id,{method:'PUT',body:JSON.stringify(body)})}else{if(!body.username||!body.password){alert('用户名和密码必填');return}await api('/users',{method:'POST',body:JSON.stringify(body)})}document.getElementById('userUsername').disabled=false;hideModal('userModal');loadUsers()}
