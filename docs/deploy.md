...
> **开发注意**：D1 的 `db.exec()` 仅支持**单行** SQL；`worker/src/database.ts` 中所有 `CREATE TABLE` 须写在一行内，多行语句会导致初始化失败、全站返回「服务器内部错误」。
...
| `CORS_ALLOWED_ORIGINS` | 否 | 额外允许的浏览器 Origin，逗号分隔完整 URL（如 `https://app.example.com`）；默认可从 `VITE_EMAIL_DOMAIN` / `MAIL_DOMAIN` 推导 |
...
- **未配置时**：登录与管理后台登录返回 **503**（`服务器未配置 SESSION_SECRET` / `未配置 SESSION_SECRET`），而非 401。
...
完整工具列表与 Cursor 配置见 **[mcp.md](./mcp.md)** 与 [packages/mcp README](https://github.com/jia0327/zmailr/blob/main/packages/mcp/README.md)。MCP 为客户端可选组件，**不影响 Worker 部署**。
...
1. 确认 `GET /api/config` 返回 200（若全站 500，检查 Worker 日志中的 D1 初始化错误）。
2. 访问 `https://你的域名/login`，使用默认演示账号 **guest / guest**（首次迁移自动创建），或在管理后台创建其它账号。
3. 在 Dashboard → **API 密钥** 创建 Bearer Token（所有程序化 API 均需 Bearer，不支持匿名）。
4. 确认 `https://你的域名/openapi.json` 与 `/api-docs` 可访问。
5. 检查依赖连通性：`GET /api/public/status` 应返回 `status: "ok"`，且 `checks.d1.ok`、`checks.r2.ok` 为 `true`（未配置 Brevo 时 `checks.brevo.configured: false` 仍属正常）。响应字段说明见 [api.md § 公开状态](./api.md#public-status)。
6. 运行 API 验证脚本（将 `<token>` 与域名替换为你的值）：
...
7. 管理后台：`https://你的域名/{ADMIN_PATH}`，使用 `ADMIN_PASSWORD` 登录。
8. （可选）按 [mcp.md](./mcp.md) 配置 `@zmailr/mcp` 供 Cursor 使用。
