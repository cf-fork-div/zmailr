...
完整安全说明见 **[security.md](./security.md)**。用户发信配额、Legacy Token 日上限均在**管理后台**配置，无需额外 GitHub Variables。
...
4. 检查依赖连通性：`GET /api/public/status` 应返回 `status: "ok"`，且 `checks.d1.ok`、`checks.r2.ok` 为 `true`（未配置 Brevo 时 `checks.brevo.configured: false` 仍属正常）。响应字段说明见 [api.md § 公开状态](./api.md#public-status)。
5. 运行 API 验证脚本（将 `<token>` 与域名替换为你的值）：
...
6. 管理后台：`https://你的域名/{ADMIN_PATH}`，使用 `ADMIN_PASSWORD` 登录。
7. （可选）按 [mcp.md](./mcp.md) 配置 `@zmailr/mcp` 供 Cursor 使用。
...
| [security.md](./security.md) | 安全模型、环境变量、Legacy Token、部署检查清单 |
