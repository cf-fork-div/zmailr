...
| 功能 | 鉴权 | 说明 |
|------|------|------|
| Web Dashboard（收件箱、发件箱、规则等） | Session Cookie | 须登录；演示账号由管理后台 **用户** 标签创建 |
| `POST /api/user/mailboxes` | Session 或 Bearer（`mail`） | 为当前用户创建临时收件箱 |
| Web 发信 `POST /api/user/send` | Session Cookie | 须登录 |
| 程序化 API（`/api/lease`、`/api/mail`、`/api/send` 等） | Bearer Token | Dashboard → API 密钥创建的用户 Token；**均需认证** |
...
在管理后台 **用户** 标签配置每位用户的 `daily_send_quota`（`-1` 为无限）。详见 [admin-guide.md](./admin-guide.md)。
...
未识别为登录用户或用户 Bearer Token 的请求走**全局 IP 限流**（默认 60 req/min）。
...
`from` 须为**你名下**、域名正确且未过期的临时邮箱；未指定时使用 `no-reply@你的域名`。
...
- Session：HMAC 签名 Cookie（派生自 `ADMIN_PASSWORD`）；admin Cookie 作用域 `Path=/{ADMIN_PATH}`
- 登录：`POST /api/auth/login` 与 Admin 登录均有 IP 暴力破解防护
...
- [admin-guide.md](./admin-guide.md) — 维护模式、请求监控、审计日志
- [brevo-setup.md](./brevo-setup.md) — 出站发信配置
- [mcp.md](./mcp.md) — MCP 集成
- [deploy.md](./deploy.md) — 部署指南
- [security.md](./security.md) — 安全模型与检查清单
- [文档首页](./) — 文档分类导航
