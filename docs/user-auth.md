...
**Legacy** `api_tokens`：在管理后台 **系统设置** 配置 **Legacy Token 日发信上限**（按 IP，默认 50，`-1` 不限），与用户 **日发信配额** 独立。详见 [security.md](./security.md)。
...
- 用户 API Token：仅存 SHA-256 哈希
- Legacy `api_tokens`：存 SHA-256 哈希；列表仅掩码；不可 `GET /api/mailboxes` 枚举
