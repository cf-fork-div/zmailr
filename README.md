...
<div style="background-color: #2d2d2d; color: #ffffff; padding: 15px; border-radius: 5px; margin: 15px 0;">
  ...
  <ol>
    ...
    <li>点击 <strong>New repository secret</strong> 并添加以下七个密钥：
      <ul>
...
通过 Brevo 从 `no-reply@你的域名` 发送邮件（需完成 [Brevo 发信配置](docs/brevo-setup.md)）。
...
`/api/send` 默认使用 [Brevo](https://www.brevo.com/) Transactional Email API。完整步骤见 **[docs/brevo-setup.md](docs/brevo-setup.md)**（注册、域名 SPF/DKIM/DMARC、发件人、API Key、关闭 IP 限制、GitHub Secret、测试）。

简要步骤：

1. 注册 Brevo 免费计划（约 300 封/天）
2. 添加并验证发信域名（Cloudflare DNS）
3. 添加发件人 `no-reply@你的域名`（显示名 `zMailR`）
4. 创建 API Key，关闭 **Authorized IPs** 限制（Worker 必需）
5. GitHub Secret 添加 `BREVO_API_KEY` 后重新部署

> 发件地址固定为 `no-reply@你的域名`。未配置 Brevo 时可选用 MailChannels（`MAILCHANNELS_API_KEY`）作为回退。

---

<details>
<summary>📤 MailChannels 发信配置（旧方案 / 回退）</summary>

`/api/send` 在未配置 `BREVO_API_KEY` 时可回退到 [MailChannels Email API](https://www.mailchannels.com/)。自 2024 年起需 **API Key + Domain Lockdown DNS**，不再支持仅 `cfid=` 的无密钥发信。

### 1. 注册 MailChannels 并创建 API Key

1. 在 [MailChannels 注册](https://signup.mailchannels.net/pricing/signup?txHandle=email-api-free)（免费计划约 100 封/天）
2. 登录 [MailChannels Console](https://dash.mailchannels.net) → 创建 API Key（scope: `api`）
3. 记下 **Account ID**（用于 DNS `auth=`）和 **API Key**（写入 GitHub Secret `MAILCHANNELS_API_KEY`）

### 2. DNS（Cloudflare）

在发信域名（与 `VITE_EMAIL_DOMAIN` / `MAIL_DOMAIN` 一致）添加：

| 类型 | 名称 | 内容 | 说明 |
|------|------|------|------|
| TXT | `_mailchannels` | `v=mc1 auth=你的MailChannels账户ID` | Domain Lockdown（勿再用 `cfid=`） |
| TXT | `@` | `v=spf1 a mx include:relay.mailchannels.net ~all` | SPF；若已有 SPF，合并 `include:relay.mailchannels.net` |

### 3. 部署与测试

1. GitHub Secrets 添加 `MAILCHANNELS_API_KEY` 后重新部署
2. 在 **Email Routing** 中确认域名已启用（收信与发信可共用同一域名）
3. 通过 `/admin` 发信记录或 `POST /api/send` 测试；失败时查看 Worker 日志
